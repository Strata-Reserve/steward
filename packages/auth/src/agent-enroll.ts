...
export function buildEnrollCanonicalString(input: {
  ...
  return [AGENT_ENROLL_DOMAIN, input.agentId, input.nonce, String(input.issuedAt)].join("\n");
}
...
export async function issueEnrollChallenge(
  ...
  return {
    ...
    challenge: {
      ...
      issuedAt,
      expiresAt,
      canonicalString: buildEnrollCanonicalString({ agentId: trimmed, nonce, issuedAt }),
    },
  };
}
 ...
 */
export type AgentSignerResolver = (agentId: string) => Promise<ResolvedAgentSigner[]>;
...
export async function verifyEnrollResponse(
  ...
  try {
    ...
    return { ok: false, error: "challenge store unavailable", code: "challenge_not_found" };
  }
  if (issuedAtRaw === null) {
    return { ok: false, error: "challenge not found or already used", code: "challenge_not_found" };
  }
  const issuedAt = Number(issuedAtRaw);
  if (!Number.isFinite(issuedAt)) {
    return { ok: false, error: "challenge malformed", code: "challenge_not_found" };
  }
  ...
  try {
    ...
    return { ok: false, error: "no active enrollment key for agent", code: "no_active_key" };
  }
  const active = (signers ?? []).filter(
    (s) => s && s.status === "active" && s.keyType === "p256" && typeof s.publicKey === "string",
  );
  if (active.length === 0) {
    return { ok: false, error: "no active enrollment key for agent", code: "no_active_key" };
  }
