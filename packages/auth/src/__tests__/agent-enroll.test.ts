...
function resolverFor(map: Record<string, ResolvedAgentSigner[]>): AgentSignerResolver {
...
describe("agent enrollment (keypair-only self-auth)", () => {
  test("happy path: agent signs the challenge and enrolls", async () => {
    ...
    const resolve = resolverFor({
      [agentId]: [{ publicKey: kp.publicKeySpkiBase64, status: "active", keyType: "p256" }],
    });
    ...
    expect(issued.challenge.canonicalString.startsWith(AGENT_ENROLL_DOMAIN)).toBe(true);
    const signature = await signP256(kp.privateKey, issued.challenge.canonicalString);
  ...
  test("expiry: a stale challenge is rejected", async () => {
    ...
    const t0 = 1_000_000;
    const issued = await issueEnrollChallenge(store, agentId, { now: t0, ttlMs: 1000 });
    if (!issued.ok) throw new Error("challenge not issued");
    const signature = await signP256(kp.privateKey, issued.challenge.canonicalString);
  ...
  test("wrong key: a signature from another keypair fails closed", async () => {
    ...
    const resolve = resolverFor({
      [agentId]: [{ publicKey: legit.publicKeySpkiBase64, status: "active", keyType: "p256" }],
    });
    const issued = await issueEnrollChallenge(store, agentId);
    if (!issued.ok) throw new Error("challenge not issued");
    const badSig = await signP256(attacker.privateKey, issued.challenge.canonicalString);
  ...
  test("hmac signer cannot self-enroll (no asymmetric proof)", async () => {
    ...
    const resolve = resolverFor({
      [agentId]: [{ publicKey: kp.publicKeySpkiBase64, status: "active", keyType: "hmac" }],
    });
    const issued = await issueEnrollChallenge(store, agentId);
    if (!issued.ok) throw new Error("challenge not issued");
    const signature = await signP256(kp.privateKey, issued.challenge.canonicalString);
  ...
  test("revoked (inactive) key cannot enroll", async () => {
    ...
    const resolve = resolverFor({
      [agentId]: [{ publicKey: kp.publicKeySpkiBase64, status: "revoked", keyType: "p256" }],
    });
    const issued = await issueEnrollChallenge(store, agentId);
    if (!issued.ok) throw new Error("challenge not issued");
    const signature = await signP256(kp.privateKey, issued.challenge.canonicalString);
  ...
  test("key rotation: verify tries each active key until one matches", async () => {
    ...
    const resolve = resolverFor({
      [agentId]: [
        { publicKey: oldKp.publicKeySpkiBase64, status: "active", keyType: "p256" },
        { publicKey: newKp.publicKeySpkiBase64, status: "active", keyType: "p256" },
      ],
    });
    const issued = await issueEnrollChallenge(store, agentId);
    if (!issued.ok) throw new Error("challenge not issued");
    // Sign with the NEW (second) key — must still match.
    const signature = await signP256(newKp.privateKey, issued.challenge.canonicalString);
