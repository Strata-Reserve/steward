...
describe("shared email login challenge", () => {
  ...
  it("binds both credentials to normalized email and tenant", async () => {
    ...
    expect(await auth.verifyEmailLoginCode("bound@example.com", code, "tenant-b")).toMatchObject({
      valid: false,
    });
    expect(await auth.verifyMagicLink(token, "other@example.com", "tenant-a")).toMatchObject({
      valid: false,
    });
    auth.destroy();
  });
  ...
  it("normalizes mixed-case email before companion-code verification", async () => {
    ...
    expect(await auth.verifyEmailLoginCode("MIXED.COM", code, "tenant-a")).toMatchObject({
      valid: true,
      email: "mixed.com",
    });
    auth.destroy();
  ...
  it("expires the link, code, and polling status on the same short TTL", async () => {
    ...
    const issued = await auth.sendMagicLink("ttl@example.com", { tenantId: "tenant-a" });
    const { token, code } = credentials();
    await Bun.sleep(20);

    expect(await auth.verifyMagicLink(token, "ttl@example.com", "tenant-a")).toMatchObject({
      valid: false,
    });
    expect(await auth.verifyEmailLoginCode("ttl@example.com", code, "tenant-a")).toMatchObject({
      valid: false,
    });
  ...
  it("invalidates the prior challenge when the same target resends", async () => {
    const { auth, credentials } = captureAuth();
    const first = await auth.sendMagicLink("resend@example.com", { tenantId: "tenant-a" });
    const old = credentials();
    await auth.sendMagicLink("resend@example.com", { tenantId: "tenant-a" });

    expect(await auth.verifyMagicLink(old.token, "resend@example.com", "tenant-a")).toMatchObject({
      valid: false,
    });
    expect(
      await auth.verifyEmailLoginCode("resend@example.com", old.code, "tenant-a"),
    ).toMatchObject({
      valid: false,
    });
    expect(await auth.getEmailLoginStatus(first.challengeId, first.pollSecret)).not.toMatchObject({
      status: "pending",
    });
    auth.destroy();
  });
