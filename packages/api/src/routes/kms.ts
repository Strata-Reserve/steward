 ...
 */
function requireKmsAgent(c: Context<{ Variables: AppVariables }>): KmsPrincipal | null {
...
function deny(c: Context<{ Variables: AppVariables }>): Response {
  return c.json<ApiResponse>({ ok: false, error: "KMS requires an agent token" }, 403);
}

function badRequest(c: Context<{ Variables: AppVariables }>, error: string): Response {
  return c.json<ApiResponse>({ ok: false, error }, 400);
}

// ── wire helpers ─────────────────────────────────────────────────────────────

function parseKeyIdParam(c: Context<{ Variables: AppVariables }>): string | null {
...
function deriveKey(
  ...
  const info = Buffer.from(`${HKDF_DOMAIN}|${use}|${keyId}|v${version}`, "utf8");
...
function ed25519PublicRaw(root: Buffer, keyId: string, version: number): Buffer {
...
kmsRoutes.post("/keys", async (c) => {
  ...
  if (typeof keyId !== "string" || !KEY_ID_RE.test(keyId)) {
    return badRequest(c, "keyId is required and must match the KMS key-id charset");
  }
  ...
  try {
    ...
    return c.json<ApiResponse>({ ok: false, error: "Failed to create KMS key" }, 500);
  }
});
...
kmsRoutes.get("/keys/:keyId/versions", async (c) => {
  ...
  const rows = await listKeyRows(principal.tenantId, secretNameFor(principal.agentId, keyId));
...
kmsRoutes.post("/keys/:keyId/encrypt", async (c) => {
  ...
  const body = await safeJsonParse<{ plaintext_b64?: unknown; aad_b64?: unknown }>(c);
  if (!body) return badRequest(c, "Invalid JSON in request body");
  const plaintext = decodeB64Field(body.plaintext_b64);
  if (!plaintext) return badRequest(c, "plaintext_b64 is required and must be valid base64");
...
kmsRoutes.post("/keys/:keyId/decrypt", async (c) => {
  ...
  if (!ciphertext || !nonce || !authTag) {
    return badRequest(c, "ciphertext_b64, nonce_b64 and auth_tag_b64 are required valid base64");
  }
  if (nonce.length !== AEAD_NONCE_BYTES) return badRequest(c, "nonce must be 12 bytes");
  if (authTag.length !== AEAD_TAG_BYTES) return badRequest(c, "auth tag must be 16 bytes");
  let aad: Buffer | undefined;
  if (body.aad_b64 !== undefined) {
    const decoded = decodeB64Field(body.aad_b64);
    if (!decoded) return badRequest(c, "aad_b64 must be valid base64");
  ...
  if (!current) return c.json<ApiResponse>({ ok: false, error: "key not found" }, 404);
...
kmsRoutes.post("/keys/:keyId/hmac", async (c) => {
  ...
  if (!data) return badRequest(c, "data_b64 is required and must be valid base64");
...
kmsRoutes.post("/keys/:keyId/hmac/verify", async (c) => {
  ...
  const body = await safeJsonParse<{ data_b64?: unknown; tag_b64?: unknown }>(c);
  if (!body) return badRequest(c, "Invalid JSON in request body");
  const data = decodeB64Field(body.data_b64);
  const tag = decodeB64Field(body.tag_b64);
  if (!data || !tag) return badRequest(c, "data_b64 and tag_b64 are required valid base64");

  const rows = await listKeyRows(principal.tenantId, secretNameFor(principal.agentId, keyId));
  if (!currentRow(rows)) return c.json<ApiResponse>({ ok: false, error: "key not found" }, 404);
...
kmsRoutes.post("/keys/:keyId/sign", async (c) => {
  ...
  const body = await safeJsonParse<{ data_b64?: unknown; algorithm?: unknown }>(c);
  if (!body) return badRequest(c, "Invalid JSON in request body");
  const data = decodeB64Field(body.data_b64);
  if (!data) return badRequest(c, "data_b64 is required and must be valid base64");
  if (body.algorithm !== SUPPORTED_ALGORITHM) {
    return badRequest(c, `unsupported algorithm (only ${SUPPORTED_ALGORITHM} is implemented)`);
  }
...
kmsRoutes.post("/keys/:keyId/verify", async (c) => {
  ...
  if (!data || !signature) {
    return badRequest(c, "data_b64 and signature_b64 are required valid base64");
  }
  if (body.algorithm !== SUPPORTED_ALGORITHM) {
    return badRequest(c, `unsupported algorithm (only ${SUPPORTED_ALGORITHM} is implemented)`);
  }
