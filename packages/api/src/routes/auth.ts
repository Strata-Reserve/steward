...
type RefreshRotationResult =
  | {
      ...
      // Returned so the route can canonicalize the token SUBJECT *outside* this
      // DB transaction (subject resolution provisions a wallet via the vault and
      // must never run inside a transaction). See the /refresh handler.
...
async function rotateRefreshTokenForUserSession(raw: string): Promise<RefreshRotationResult> {
  ...
  return db.transaction(async (tx): Promise<RefreshRotationResult> => {
    ...
    // Fail-closed email claim: only embed `email`/`emailVerified` when the row
    // actually carries a VERIFIED email (buildSessionIdentityClaims), instead of
    // echoing any email present on the row. Pure/synchronous, so it is safe here
    // inside the transaction. The SUBJECT is canonicalized by the /refresh route
    // after this transaction commits.
    const newAccessToken = await createSessionToken(walletAddress, record.tenantId, {
      ...(user ? buildSessionIdentityClaims(user) : { userId: record.userId }),
      ...sessionClaims,
    });
    ...
    return { status: "valid", record, newAccessToken, newRefreshToken, user, sessionClaims };
  });
}
 ...
 *
 * NOTE: named `buildSessionIdentityClaims` to disambiguate from upstream's
 * async `buildIdentityClaims(userId, tenantId)`, which builds the claim set for
 * the separate tenant-scoped IDENTITY token. This one shapes the claims for the
 * human SESSION token and is pure/synchronous.
 */
export function buildSessionIdentityClaims(
  user: typeof users.$inferSelect,
): Record<string, unknown> {
...
async function completeEmailAuth(
  ...
  const response = await buildAuthOrMfaResponse(
    ...
    walletAddress ?? "",
    {
      // `emailVerified` was just set true above; reflect that for claim
      // building so the token carries the verified-email claim (fail-closed
      // helper: it only emits `email`/`emailVerified` when actually verified).
      ...buildSessionIdentityClaims({ ...user, email, emailVerified: true }),
      authMethod: "email",
    },
...
auth.post("/verify", async (c) => {
  ...
  // wallet so the token SUBJECT does not rotate across logins. The descriptive
  // `address` fields below still report the raw SIWE credential; only the
  // subject is canonicalized. Email claim stays absent for a pure wallet user
  // (fail-closed via buildSessionIdentityClaims).
...
auth.post("/refresh", async (c) => {
  ...
  const { record, newRefreshToken, user: refreshUser, sessionClaims } = rotatedRefresh;
  let { newAccessToken } = rotatedRefresh;
  const db = getDb();
  ...
  //    wallet → use it directly (already the subject minted in the rotation).
  ...
  //
  // This runs OUTSIDE the rotation transaction on purpose: resolving the
  // canonical subject provisions a wallet through the vault, which must never
  // happen inside an open DB transaction.
  if (refreshUser && !refreshUser.email) {
    const subject = await resolveExternalWalletSubject(
      refreshUser,
      refreshUser.walletAddress ?? "",
    );
    if (subject !== (refreshUser.walletAddress ?? "")) {
      // Re-mint the access token on the canonical subject, preserving the
      // fail-closed claim set and any MFA/session claims carried by rotation.
      newAccessToken = await createSessionToken(subject, record.tenantId, {
        ...buildSessionIdentityClaims(refreshUser),
        ...sessionClaims,
      });
    }
  }
...
auth.post("/passkey/register/verify", async (c) => {
  ...
  return authExchangeJson(
    ...
    await buildAuthOrMfaResponse(
      ...
      walletAddress ?? "",
      {
        // emailVerified was set true above on register; embed the verified
        // email via the fail-closed claim builder.
        ...buildSessionIdentityClaims({ ...user, email, emailVerified: true }),
        authMethod: "passkey",
      },
      {
...
auth.post("/passkey/login/verify", async (c) => {
  ...
  return authExchangeJson(
    ...
    await buildAuthOrMfaResponse(
      ...
      walletAddress ?? "",
      {
        // Passkey login implies a verified email-keyed account; embed the
        // verified email via the fail-closed claim builder.
        ...buildSessionIdentityClaims({ ...user, email, emailVerified: true }),
        authMethod: "passkey",
      },
      {
  ...
  try {
    ...
    return {
      ...
      response: await buildAuthOrMfaResponse(
        ...
        walletAddress ?? "",
        {
          ...buildSessionIdentityClaims({ ...user, email, emailVerified }),
          authMethod: "oauth",
        },
        { id: user.id, email, walletAddress },
        c,
