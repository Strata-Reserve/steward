...
export function signProviderExecutionCommitmentV2(
  ...
  return base64Url(
    createHmac("sha256", active.key).update(providerExecutionSignatureInput(commitment)).digest(),
  );
}
...
export function verifyProviderExecutionCommitmentV2(
  ...
  const expected = base64Url(
    createHmac("sha256", entry.key).update(providerExecutionSignatureInput(commitment)).digest(),
  );
