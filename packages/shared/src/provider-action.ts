...
class JsonScanner {
  ...
  parseTopLevel(): JsonValue {
    ...
    const v = this.value();
    this.ws();
    if (this.i !== this.s.length) fail("CANON_JSON_SYNTAX_INVALID", "trailing content after JSON");
    return v;
  }
  ...
  private value(): JsonValue {
    if (this.i >= this.s.length) fail("CANON_JSON_SYNTAX_INVALID", "unexpected end of input");
  ...
  private string(): string {
    ...
    let out = "";
    for (;;) {
      if (this.i >= this.s.length) fail("CANON_JSON_SYNTAX_INVALID", "unterminated string");
      const c = this.s[this.i];
      ...
      if (code < 0x20) fail("CANON_JSON_SYNTAX_INVALID", "unescaped control character in string");
      if (c === "\\") {
        this.i++;
        out += this.escape();
        continue;
  ...
  private hex4(): number {
    if (this.i + 4 > this.s.length) fail("CANON_JSON_SYNTAX_INVALID", "truncated \\u escape");
    let v = 0;
    for (let k = 0; k < 4; k++) {
  ...
  private expect(ch: string): void {
    if (this.s[this.i] !== ch) fail("CANON_JSON_SYNTAX_INVALID", `expected '${ch}'`);
    this.i++;
  }
}
...
export function parseStrictInteger(lexeme: string): JsonInteger {
  if (lexeme.includes(".") || lexeme.includes("e") || lexeme.includes("E"))
    fail("CANON_NUMBER_FORMAT_UNSUPPORTED", `non-integer number lexeme '${lexeme}'`);
  if (lexeme.includes("+")) fail("CANON_NUMBER_FORMAT_UNSUPPORTED", "leading plus not allowed");
...
export function strictParseJson(text: string): JsonValue {
  if (text.charCodeAt(0) === 0xfeff) fail("CANON_INVALID_UTF8", "BOM not allowed");
...
function serializeValue(v: unknown): string {
  ...
  if (t === "bigint") fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "bigint is not a JSON value");
  if (t === "undefined") fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "undefined is not a JSON value");
  ...
  if (t === "object") {
    ...
    if (!PLAIN_OBJECT_PROTOS.includes(proto))
      fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "non-plain object (Date/Map/Set/class) not allowed");
...
function serializeNumber(n: number): string {
  if (!Number.isFinite(n)) fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "NaN/Infinity not a JSON value");
  ...
  if (!Number.isSafeInteger(n))
    fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "only safe-integer numbers are serialized");
...
function serializeArray(arr: unknown[]): string {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (!(i in arr)) fail("CANON_RUNTIME_VALUE_UNSUPPORTED", "sparse array hole not allowed");
...
function serializeObject(obj: Record<string, unknown>): string {
  ...
  for (const k of keys) {
    ...
    if (val === undefined) fail("CANON_RUNTIME_VALUE_UNSUPPORTED", `member '${k}' is undefined`);
 ...
 */
function toCanonicalActionObject(a: GithubCanonicalActionV1): Record<string, unknown> {
  return {
    profile: a.profile,
    method: a.method,
    origin: a.origin,
...
export function canonicalizeOrigin(raw: string): string {
  ...
  if (/[\u0000-\u0020\u007f]/.test(raw)) fail("CANON_ORIGIN_INVALID", "control/space in origin");

  const schemeMatch = /^([A-Za-z][A-Za-z0-9+.-]*):\/\//.exec(raw);
  if (!schemeMatch) fail("CANON_ORIGIN_INVALID", "origin missing scheme://");
  const scheme = schemeMatch[1].toLowerCase();
  if (scheme !== "https") fail("CANON_ORIGIN_SCHEME_UNSUPPORTED", `scheme '${scheme}' not https`);
  ...
  if (rest.includes("?")) fail("CANON_ORIGIN_INVALID", "query not allowed in origin");
  if (rest.includes("#")) fail("CANON_ORIGIN_INVALID", "fragment not allowed in origin");

  // Split host[:port] from an optional trailing path.
  const slash = rest.indexOf("/");
  const authority = slash === -1 ? rest : rest.slice(0, slash);
  const path = slash === -1 ? "" : rest.slice(slash);
  if (path !== "" && path !== "/") fail("CANON_ORIGIN_INVALID", "origin path must be empty or '/'");
  ...
  if (colon !== -1) {
    ...
    if (!/^[0-9]+$/.test(portStr)) fail("CANON_ORIGIN_PORT_UNSUPPORTED", "invalid port");
    if (portStr !== "443") fail("CANON_ORIGIN_PORT_UNSUPPORTED", `nondefault port ${portStr}`);
  }

  // Host must be ASCII, may have exactly one terminal dot which we strip.
  if (/%/.test(host)) fail("CANON_ORIGIN_HOST_INVALID", "percent escape in host");
  if (/[^\x00-\x7f]/.test(host)) fail("CANON_ORIGIN_HOST_INVALID", "non-ASCII host");
  let h = host.toLowerCase();
  if (h.endsWith("..")) fail("CANON_ORIGIN_HOST_INVALID", "multiple terminal dots");
  if (h.endsWith(".")) h = h.slice(0, -1);
  // Reject IP literals (v4 dotted-decimal, or bracketed v6).
  if (h.startsWith("[")) fail("CANON_ORIGIN_HOST_INVALID", "IP literal host");
  if (/^[0-9]+(\.[0-9]+)*$/.test(h)) fail("CANON_ORIGIN_HOST_INVALID", "numeric/IPv4 host");
  // Validate DNS labels.
  for (const label of h.split(".")) {
    if (!/^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/.test(label))
      fail("CANON_ORIGIN_HOST_INVALID", `invalid DNS label '${label}'`);
  }
  if (h !== "api.github.com") fail("CANON_ORIGIN_NOT_ALLOWED", `host '${h}' not allowed`);
...
export function normalizePath(raw: string): string {
  if (typeof raw !== "string") fail("CANON_PATH_INVALID", "path must be a string");
  if (raw.length === 0 || raw[0] !== "/") fail("CANON_PATH_INVALID", "path must begin with '/'");
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (c < 0x20 || c === 0x7f) fail("CANON_PATH_FORBIDDEN_BYTE", "control/DEL in path");
  ...
  if (raw.endsWith("/")) fail("CANON_PATH_EMPTY_SEGMENT", "trailing slash not allowed");

  const segments = raw.slice(1).split("/");
  const outSegments: string[] = [];
  for (const seg of segments) {
    if (seg.length === 0) fail("CANON_PATH_EMPTY_SEGMENT", "empty path segment ('//')");
    if (seg === "." || seg === "..") fail("CANON_PATH_TRAVERSAL", "literal dot segment");
...
function normalizePathSegment(seg: string): string {
  ...
  for (let i = 0; i < seg.length; ) {
    const c = seg[i];
    if (c === "%") {
      if (i + 2 >= seg.length) fail("CANON_PATH_PERCENT_INVALID", "truncated percent escape");
      ...
      if (
        ...
        fail("CANON_PATH_ENCODED_AMBIGUITY", `encoded byte 0x${hex} not allowed`);
 ...
 */
export function canonicalizeQueryPairs(pairs: ReadonlyArray<QueryPair>): Array<[string, string]> {
  const seen = new Set<string>();
  ...
  for (const pair of pairs) {
    ...
    if (seen.has(name)) fail("CANON_QUERY_DUPLICATE_KEY", `duplicate query name '${name}'`);
    seen.add(name);
...
function assertQueryScalar(s: string): void {
  for (let i = 0; i < s.length; i++) {
    ...
    if (c < 0x20 || c === 0x7f) fail("CANON_QUERY_VALUE_INVALID", "control/DEL in query");
  }
...
export function parseRawQuery(raw: string): QueryPair[] {
  ...
  if (raw.includes("#")) fail("CANON_QUERY_SYNTAX_AMBIGUOUS", "fragment in query");
  if (raw.includes(";")) fail("CANON_QUERY_SYNTAX_AMBIGUOUS", "semicolon separator in query");
  const out: QueryPair[] = [];
  for (const part of raw.split("&")) {
    if (part.length === 0) fail("CANON_QUERY_SYNTAX_AMBIGUOUS", "empty query segment");
...
function percentDecodeQuery(token: string): string {
  ...
  for (let i = 0; i < token.length; ) {
    const c = token[i];
    if (c === "%") {
      if (i + 2 >= token.length) fail("CANON_QUERY_PERCENT_INVALID", "truncated percent escape");
      ...
      if (code > 0x7f) fail("CANON_QUERY_VALUE_INVALID", "raw non-ASCII byte in query token");
      bytes.push(code);
      i += 1;
    }
  }
  try {
    ...
    fail("CANON_QUERY_PERCENT_INVALID", "percent-decoded bytes are not valid UTF-8");
  }
}
...
export function canonicalizeHeaders(raw: ReadonlyArray<[string, string]>): Array<[string, string]> {
  ...
  const entries = [...byName.entries()].sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
...
function assertHeaderValueClean(v: string): void {
  for (let i = 0; i < v.length; i++) {
    ...
    if (c < 0x20 || c === 0x7f) fail("CANON_HEADER_INVALID", "control/DEL in header value");
  }
}
...
function canonicalizeHeaderValue(name: string, value: string): string {
  switch (name) {
    case "accept":
      if (value !== "application/vnd.github+json" && value !== "application/json")
...
export function canonicalizeContentType(raw: string): string {
  ...
  if (media !== "application/json" && media !== "application/vnd.github+json")
    fail("CANON_BODY_CONTENT_TYPE_UNSUPPORTED", `unsupported media type '${media}'`);
  const params = parts.slice(1).filter((p) => p.length > 0);
  const seenParam = new Set<string>();
  for (const p of params) {
    const eq = p.indexOf("=");
    if (eq === -1) fail("CANON_BODY_CONTENT_TYPE_INVALID", "malformed content-type parameter");
    const pname = p.slice(0, eq).trim().toLowerCase();
    const pvalRaw = p.slice(eq + 1).trim();
    if (seenParam.has(pname))
      fail("CANON_BODY_CONTENT_TYPE_INVALID", `duplicate content-type parameter '${pname}'`);
    seenParam.add(pname);
    if (pname !== "charset")
      fail("CANON_BODY_CONTENT_TYPE_INVALID", `unsupported content-type parameter '${pname}'`);
    ...
    if (pvalRaw.toLowerCase() !== "utf-8")
      fail("CANON_BODY_CONTENT_TYPE_INVALID", `unsupported charset '${pvalRaw}'`);
  }
  return media;
}
...
export const GOLDEN_VECTORS: GoldenVector[] = [
  {
    ...
    actionDigest: "sha256:effa84639ed9c9b0b2c01b65bd716342a25a846d9209818b194ab3d151276f3a",
    requestHash: "sha256:8c0d3d5761ad6ad8ea017d3d36bd57157a7d2f5767acce8ede417d4556b377e3",
  },
  {
    id: "GV-02",
  ...
  },
  {
    ...
    actionDigest: "sha256:445b2d106b942351d69feeb8d8a7575d793ddfbc357b0cea35e66f0696432b57",
    requestHash: "sha256:ea3288afc951a71bccca01fd4e06e638998e9d86b303321545d8d4afe4fb8f38",
  },
  {
    id: "GV-05",
    ...
    actionDigest: "sha256:13256b604d25d80a338c192e7a1848f7b33917434b6014906657123008c61bcb",
    requestHash: "sha256:72b69a61a6c11515a025086f041dcd021e1c9abd1a4a5706e9420768c3d6bc49",
  },
  {
    id: "GV-06",
    ...
    actionDigest: "sha256:61d58545cb091a9d74f2ef062dcb98afd6ecf6dbe6495da26192a2aaef9caf0d",
    requestHash: "sha256:5fb075a546f84b8e7a9134632272dfac4f9b8d1a8e3eca25d3c11c496920b798",
  },
  {
    id: "GV-07",
    ...
    requestHash: "sha256:42b54c0c181aabd929519dd674d635f677dfd20574fd3e0df1673a4b131fcead",
  },
  {
    id: "GV-08",
    ...
    actionDigest: "sha256:04bf6b053c634f0dc6a216dc718841ddfffec4bb29c75ca5f1215067335960f4",
    requestHash: "sha256:b679c7cb25bdb5d5ea7ac673f0a6cfeb239ca036f3644b1e87b175195e742116",
  },
  {
    id: "GV-09",
    ...
    requestHash: "sha256:caaa283757f984567398d7c64f3ef8699e02c8b3765e67568de7eed175d197d3",
  },
  {
    id: "GV-10",
    ...
    actionDigest: "sha256:f1b5fea9a6a70c9862fd58959f98d1464d3030864b2f34f419f50ec0a4ab4ab5",
    requestHash: "sha256:d15a5781c01b2e7cf6d9ec9def521188a09eb37331270f578e278c9d56f43996",
  },
  {
    id: "GV-11",
    ...
    actionDigest: "sha256:374f75fb15c8cfd2086597a685dcc14aba668897e8519deebecb84e6ee9d6196",
    requestHash: "sha256:9015bba7077c6a973e370ba620786dbc45e3e175370b81f74b42afaeed931d51",
  },
  {
    id: "GV-12",
    ...
    actionDigest: "sha256:1eb38fc53c5e424aff0e3f0cb4680b9078fab110bb1c04eb22738e6e41e8031c",
    requestHash: "sha256:d43cdda870f9c00deedb72c76523494faec9ed6b582d3fb6cd8612046279e901",
  },
  {
    id: "GV-13",
    ...
    actionDigest: "sha256:0670c3e5ccdffcb2acf47ef76d22183bd02d8b41a320990b4ff5be1b8ab6751c",
    requestHash: "sha256:4b15779aa787faa36bd448e33df3bee496bd1bde98fa7f29d4b3baa6e4fe1af9",
  },
  {
    id: "GV-14",
    ...
    actionDigest: "sha256:b5df21376f2f76310889d5c00d063a28e4e7b37166df5e5f24799a3502bac148",
    requestHash: "sha256:e9fdbd5d464d8f7a81a4e68fb9dfe2d3ee068146f62ce7a2217c08d7bc6cae31",
  },
  {
    id: "GV-15",
    ...
    actionDigest: "sha256:aa31a6681277932dbbbaca01bd7811a1317428bf76e04a7a02c496d7f2065148",
    requestHash: "sha256:e3854994f534286310f4174ff80be6bd4124adb73026884219c722d0141de74b",
  },
  {
    id: "GV-16",
    ...
    canonicalActionBytes:
      '{"canonicalBody":null,"method":"GET","normalizedPath":"/repos/octo/~hello/issues","orderedQueryPairs":[],"origin":"https://api.github.com","profile":"github.provider-action.v1","selectedHeaders":[]}',
    actionDigest: "sha256:f7a1a6dc6f0f4b2ccc8f48bfa2e270e06c483326c2933ce4c88d45ba2313ad1f",
    requestHash: "sha256:0533dc5342d9faa71bfb5c72a5f156c3f0bb0dc8dddf2f75934f2fc3560288c8",
  },
  {
    id: "GV-17",
    ...
    requestHash: "sha256:73d6bd9a9b0937e6d2ee4657a94cbbee3202ddacec1ad2be8499a988b5640d68",
  },
];

/** Build the full request envelope for a golden vector (base + its digest). */
export function goldenEnvelope(actionDigest: string): ProviderRequestEnvelopeV1 {
