...
import type { MoneroNetwork, MoneroWalletMetadata } from "@stwd/shared";
...
function decodeBase58Block(block: string, byteLength: number): Uint8Array {
  ...
  for (const char of block) {
    ...
    if (digit < 0) throw new Error("Monero address contains an invalid base58 character");
  ...
  if (num > 0n) throw new Error("Monero address base58 block overflows its byte length");
  return out;
}
...
export function encodeMoneroBase58(data: Uint8Array): string {
  ...
  for (let i = 0; i < data.length; i += FULL_BLOCK_SIZE) {
    out += encodeBase58Block(data.subarray(i, Math.min(i + FULL_BLOCK_SIZE, data.length)));
  }
  return out;
}
...
export function decodeMoneroBase58(encoded: string): Uint8Array {
  ...
  if (lastBlockBytes < 0) throw new Error("Monero address has an invalid base58 length");
  ...
  if (lastBlockBytes > 0) {
    out.set(
      decodeBase58Block(encoded.slice(fullBlocks * FULL_ENCODED_BLOCK_SIZE), lastBlockBytes),
 ...
 */
export function generateMoneroWallet(network: MoneroNetwork): GeneratedMoneroWallet {
  let spendScalar = 0n;
  ...
  return {
    ...
    publicSpendKey,
    publicViewKey,
    address: moneroAddressFromPublicKeys(publicSpendKey, publicViewKey, network),
    network,
  };
}
...
export function decodeMoneroAddress(address: string): DecodedMoneroAddress {
  if (typeof address !== "string" || (address.length !== 95 && address.length !== 106)) {
    throw new Error(
  ...
  return {
    ...
    publicSpendKey,
    publicViewKey,
    ...(kind === "integrated" ? { paymentId: encodeHex(payload.subarray(65, 73)) } : {}),
  };
}

/** Validate that `address` is a valid Monero destination on `network`. */
export function assertMoneroAddress(address: string, network: MoneroNetwork): DecodedMoneroAddress {
...
export function parsePiconeroAmount(value: unknown): bigint {
  ...
  if (amount > MAX_UINT64) throw new Error("Monero amount exceeds uint64 range");
  return amount;
}
 ...
 */
export function moneroWalletScope(network: MoneroNetwork, account: number): string {
...
export function parseMoneroWalletScope(scope: string): ParsedMoneroWalletScope {
  ...
  if (!match) {
    throw new Error("Monero wallet scope must look like monero:<network>:<account>");
  }
...
function assertMoneroKeyPayload(value: unknown): asserts value is MoneroKeyPayloadV1 {
  if (typeof value !== "object" || value === null) {
    throw new Error("Monero key payload must be an object");
  }
  const payload = value as Record<string, unknown>;
  if (payload.v !== 1) throw new Error("Unsupported Monero key payload version");
...
export interface MoneroWalletBackend {
   ...
   * any mismatch.
   */
  verifyWalletKeys(payload: MoneroKeyPayloadV1, context: MoneroWalletBackendContext): Promise<void>;
  getBalance(
    payload: MoneroKeyPayloadV1,
...
function toBigIntField(value: unknown, field: string): bigint {
  ...
  throw new Error(`monero RPC response field ${field} is not a non-negative integer`);
}

function toNumberField(value: unknown, field: string): number {
  const asBigint = toBigIntField(value, field);
  if (asBigint > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`monero RPC response field ${field} exceeds safe integer range`);
  }
...
export class MoneroWalletRpcBackend implements MoneroWalletBackend {
  ...
  async getBalance(
    ...
    return this.mutex.run(async () => {
      try {
        ...
        const height = (await this.walletRpc("get_height", {})) as Record<string, unknown>;
        return {
          balancePiconero: toBigIntField(balance.balance, "balance"),
          unlockedPiconero: toBigIntField(balance.unlocked_balance, "unlocked_balance"),
          blocksToUnlock: toNumberField(balance.blocks_to_unlock ?? 0, "blocks_to_unlock"),
  ...
  async prepareTransfer(
    ...
    for (const destination of request.destinations) {
      ...
      if (destination.amountPiconero <= 0n || destination.amountPiconero > MAX_UINT64) {
        throw new Error("Monero transfer amount is out of range");
      }
    }
    if (
      request.priority !== undefined &&
      (!Number.isInteger(request.priority) || request.priority < 0 || request.priority > 3)
    ) {
      throw new Error("Monero transfer priority must be an integer between 0 and 3");
    }
    return this.mutex.run(async () => {
      try {
        await this.ensureWalletLoaded(payload, context);
        ...
        if (typeof txMetadata !== "string" || txMetadata.length === 0) {
          throw new MoneroRpcError("transfer", "response is missing tx_metadata");
        }
  ...
  private async walletRpc(method: string, params: unknown): Promise<unknown> {
    const body = stringifyWithBigints({ jsonrpc: "2.0", id: "0", method, params });
    ...
    if (response.status === 401 && this.login) {
      ...
      if (!challenge || !/^digest/i.test(challenge)) {
        throw new MoneroRpcError(method, "wallet-rpc requires an unsupported auth scheme");
      }
    ...
    if (!response.ok) {
      throw new MoneroRpcError(method, `wallet-rpc returned HTTP ${response.status}`);
    }
    ...
    if (parsed.result === undefined) {
      throw new MoneroRpcError(method, "wallet-rpc response is missing a result");
    }
