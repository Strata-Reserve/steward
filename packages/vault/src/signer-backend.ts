...
export type ThresholdScheme = "frost-secp256k1" | "frost-ed25519" | "cggmp21-ecdsa";
...
export interface SignerBackend {
   ...
   */
  generate(params: ThresholdGenerateParams, context?: KeystoreContext): Promise<ThresholdKeyRef>;
   ...
   */
  verify(ref: ThresholdKeyRef, message: Uint8Array, signature: Uint8Array): Promise<boolean>;
 ...
 */
export function assertNoRawKeyExport(backend: SignerBackend): asserts backend is SignerBackend {
  ...
  if ((backend.capabilities as { canReturnRawKey: unknown }).canReturnRawKey !== false) {
    throw new Error(
