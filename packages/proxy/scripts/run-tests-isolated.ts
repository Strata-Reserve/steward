#!/usr/bin/env bun
/**
 * Per-file isolation test runner for @stwd/proxy.
 *
 * WHY THIS EXISTS
 * ---------------
 * `bun test src/__tests__/` runs all proxy test files in ONE process, and Bun
 ...
 * SAFE TO PARALLELISE
 * -------------------
 * Each spawned process is independent. each proxy test file bootstraps its own isolated dependencies (when no real DATABASE_URL is set) and there is
...
const testDir = join(proxyRoot, "src", "__tests__");
...
console.log(
  `[isolated] ${files.length} file(s) · concurrency ${jobs} · timeout ${timeout}ms · cwd ${proxyRoot}`,
);
